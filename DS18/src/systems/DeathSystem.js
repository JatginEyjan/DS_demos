...
export class DeathSystem {
  ...
  recoverSoulsIfNeeded(roomTemplateId) {
    if (
      ...
      this.session.economySystem.addSouls(this.session.state.deathState.droppedSouls);
      this.session.log(
  ...
  handleHeroDeath(roomTemplateId) {
    ...
    if (this.session.state.deathState.deathCount === 0) {
      ...
      this.session.eventBus.emit('hero:death', {
        roomTemplateId,
        droppedSouls,
        deathCount: 1
      });
    ...
    this.session.eventBus.emit('hero:death', {
      roomTemplateId,
      droppedSouls: this.session.state.deathState.droppedSouls,
      deathCount: 2
    });
